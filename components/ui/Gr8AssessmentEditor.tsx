...
export function Gr8AssessmentEditor({ onBack, onPublish }: Gr8AssessmentEditorProps) {
    ...
    const updateChoice = (qId: string, index: number, value: string) => {
        setQuestions(questions.map(q => {
            if (q.id === qId) {
                ...
                // Smart Update: If this choice was marked correct, update its text in the Answer Key!
    ...
    const removeChoice = (qId: string, index: number) => {
        setQuestions(questions.map(q => {
            if (q.id === qId) {
                ...
                // Remove from correct answers if deleted
    ...
    const toggleAnswerKeyMode = (id: string) => {
        setQuestions(questions.map(q => q.id === id ? { ...q, isAnswerKeyMode: !q.isAnswerKeyMode } : q));
    };
    ...
    const toggleCorrectChoice = (qId: string, choiceIndex: number, type: QuestionType) => {
        setQuestions(questions.map(q => {
            if (q.id === qId) {
                ...
                if (!choiceText.trim()) return q; // Don't allow selecting empty choices
    ...
    const updateCorrectShortAnswer = (qId: string, answer: string) => {
        setQuestions(questions.map(q => q.id === qId ? { ...q, correctAnswers: [answer] } : q));
    };
    ...
    const handlePublishClick = () => {
        ...
        const validatedQuestions = questions.map(q => {
            ...
            return { ...q, hasError: qError, choiceErrors: cErrors, isAnswerKeyMode: false }; // Force close answer keys
        });
        ...
        <div className="flex flex-col w-full max-w-4xl mx-auto px-4 md:px-8 py-8 font-sans">
            ...
            <div className="flex flex-col gap-y-6">
                {questions.map((q, qIndex) => {
                    ...
                    // ==========================================
                    // STATE 1: ANSWER KEY MODE (Now Deep Blue #1E4B95)
                    // ==========================================
                        ...
                        return (
                            <div key={`ans-${q.id}`} className="bg-white border-2 border-[#1E4B95] rounded-xl p-6 shadow-md relative animate-in fade-in duration-300">
                                ...
                                <div className="mb-8">
                                    ...
                                    {q.type === 'Short Answer' && (
                                        <div className="flex flex-col gap-y-2">
                                            ...
                                            <input 
                                                ...
                                                onChange={(e) => updateCorrectShortAnswer(q.id, e.target.value)}
                                    ...
                                    {['Paragraph', 'Upload Image'].includes(q.type) && (
                                        <div className="p-6 bg-gray-50 rounded-lg border border-gray-200 text-center flex flex-col items-center justify-center gap-y-2">
                                            ...
                                            <p className="text-[#666] text-[13px] font-bold">This question type requires manual grading.</p>
                                            <p className="text-[#A0A0A0] text-[11px] font-medium max-w-sm">Just assign the maximum points above. You will be able to review and grade student submissions manually later.</p>
                                        </div>
                                    )}
                                </div>
                    ...
                    // ==========================================
                    // STATE 2: NORMAL EDITING MODE
                    // ==========================================
                    return (
                        <div key={`edit-${q.id}`} className="bg-[#F4F6F8] border border-[#D1D8DD] rounded-xl p-6 shadow-sm relative animate-in fade-in duration-300">
                            ...
                            {/* Question Body Area Based on Type */}
                            <div className="mb-6 ml-2">
                                    ...
                                    <div className="flex flex-col gap-y-3">
                                        {q.choices.map((choice, cIndex) => (
                                            <div key={cIndex} className="flex items-center gap-x-4 relative">
                                                ...
                                                {/* Delete Choice X */}
                                                {q.choices.length > 1 && (
                                                    <button onClick={() => removeChoice(q.id, cIndex)} className="text-[#0A7F93] hover:text-red-500 transition-colors outline-none p-1">
                                ...
                                {/* SHORT ANSWER (Locked Visual Preview) */}
                                {q.type === 'Short Answer' && (
                                    <input 
                                        type="text" 
                                        disabled 
                                        placeholder="Short Answer" 
                                        className="w-full p-3 text-[14px] font-medium text-gray-500 bg-transparent border-b border-gray-300 outline-none cursor-not-allowed border-dashed" 
                                    />
                                )}

                                {/* PARAGRAPH (Locked Visual Preview) */}
                                {q.type === 'Paragraph' && (
                                    <textarea 
                                        disabled 
                                        placeholder="Paragraph" 
                                        className="w-full p-3 h-24 text-[14px] font-medium text-gray-500 bg-transparent border border-gray-300 rounded-lg outline-none cursor-not-allowed resize-none border-dashed" 
                                    />
                                )}

                                {/* UPLOAD IMAGE */}
                                {q.type === 'Upload Image' && (
                                    <button className="bg-[#1A4C8B] text-white px-8 py-2.5 rounded-lg text-[13px] font-bold hover:bg-[#153a6b] transition-colors outline-none shadow-sm cursor-not-allowed opacity-80">
                                        Upload image
                                    </button>
                                )}
                            </div>

                            {/* Card Footer (Trash & Answer Key) */}
                            <div className="flex items-center justify-between border-t border-[#D1D8DD] pt-4 mt-2">
                                ...
                                <div className="flex items-center gap-x-4">
                                    {/* Show indicator if an answer key has been set */}
                                    {q.correctAnswers.length > 0 && <span className="text-[11px] font-bold text-[#1E4B95] bg-[#1E4B95]/10 px-2 py-1 rounded">Key Set ({q.points}pt)</span>}
                                    
                                    <button onClick={() => toggleAnswerKeyMode(q.id)} className="text-[#1A4C8B] font-bold text-[14px] hover:underline outline-none flex items-center gap-x-1.5">
