    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#E2E7E9] font-sans">
            ...
            <div className="flex flex-1 relative">
                ...
                {/* --- COLLAPSIBLE SIDEBAR --- */}
                {/* FIX: Added `top-0 left-0` to the fixed state so it never scrolls away on mobile! */}
                <div className={`fixed top-0 left-0 md:sticky md:top-0 z-[50] h-screen flex flex-col shrink-0 transition-all duration-300 ease-in-out overflow-hidden bg-[#F4F5F7] border-r border-[#D1D8DD] ${isSidebarOpen ? 'translate-x-0 w-[280px]' : '-translate-x-full md:translate-x-0 w-[280px] md:w-0'}`}>
                    <div className="w-[280px] h-full flex flex-col">
                        ...
                        <div className="flex-1 flex flex-col gap-y-8 pt-10 px-8">
                            <button className="flex items-center justify-start w-full gap-x-4 text-[16px] font-bold text-[#222] transition-all hover:drop-shadow-lg bg-transparent border-none cursor-pointer p-0 text-left outline-none">
                            ...
                            <button
                                ...
                                className="flex items-center justify-start w-full gap-x-4 text-[16px] font-bold text-[#222] transition-all hover:drop-shadow-lg bg-transparent border-none cursor-pointer p-0 text-left outline-none">
                ...
                <div className="flex-1 flex flex-col relative w-full">
                    ...
                    {/* --- TOP HEADER --- */}
                    <div className="p-4 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shrink-0">
                                ...
                                <div className="absolute top-full left-0 w-full bg-white mt-1 border border-[#D1D8DD] rounded-lg shadow-xl z-50 overflow-hidden">
                                    ...
                                    {searchHistory.map((term, index) => (
                                        <button
                                            ...
                                            className="w-full text-left px-4 py-3 text-sm font-semibold text-[#444] hover:bg-[#FDF8F2] border-b last:border-0 outline-none"
                                        >
                                            {term}
                                        </button>
                                    ))}
                    ...
                    {/* --- CLASS GRID --- */}
                    <div className="px-4 md:px-8 pb-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                        ...
                        {!isFetchingClasses && classesList.map((cls) => (
                            <Link
                                ...
                                className="block group transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] outline-none"
                            >
                                <ClassCard
                        ...
                        <button
                            ...
                            className="bg-[#E9E9E9] border-2 border-dashed border-[#B0B8C1] rounded-xl p-6 flex flex-col items-center justify-center gap-y-3 hover:border-[#1A4C8B] transition-all cursor-pointer h-[180px] outline-none"
                        >
                                ...
                                {!showSuccess ? (
                                    <>
                                        ...
                                        <div className="flex justify-end gap-x-4 mt-8">
                                            <button onClick={closeModal} className="px-6 py-2 text-xs font-black text-[#ED1F24] uppercase outline-none">Cancel</button>
                                            <button onClick={handleCreateClass} className="px-10 py-2.5 text-xs font-black text-white bg-[#1A4C8B] rounded uppercase outline-none">Create</button>
                                        </div>
                                    </>
                                ) : (
                                    <div className="text-center py-4">
                                        ...
                                        <div className="bg-[#F4F6F8] border border-[#D1D8DD] rounded-lg p-6 mb-8">
                                            ...
                                            <div className="flex items-center justify-center gap-x-4">
                                                ...
                                                <button aria-label='code' onClick={handleCopyCode} className="p-2 hover:bg-gray-200 rounded-full transition-colors outline-none">
                                        ...
                                        <button onClick={closeModal} className="w-full py-3 text-xs font-black text-white bg-[#1A4C8B] rounded uppercase shadow-md hover:bg-[#153a6b] transition-all outline-none">Done</button>
                                    </div>
                                )}
                            </div>
                        </div>
